package com.mauricio.authorizacion.configuration;
...
import com.mauricio.authorizacion.repositories.RolRepository;
import com.mauricio.authorizacion.repositories.UsuarioRepository;
import com.mauricio.authorizacion.server.CustomUserDetails;
...
public class SecurityConfig {
	...
	SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http)
	    ...
	    http
	        .authorizeHttpRequests((authorize) -> authorize
	            .requestMatchers("/login", "/api/login", "/api/register").permitAll() // Agregamos /api/register
	            .requestMatchers("/admin/**").hasRole("ADMIN")
	            .anyRequest().authenticated()
	        )
	        .csrf(csrf -> csrf.disable())
	        .formLogin(form -> form
	            .loginPage("/login")           // tu página HTML
	            .permitAll()
	        )
	...
	TokenSettings tokenSettings() {
	    return TokenSettings.builder()
	        .accessTokenTimeToLive(Duration.ofHours(5)) // duración del access token
	        .refreshTokenTimeToLive(Duration.ofDays(7)) // duración del refresh token (si usas refresh)
	        .build();
	}
